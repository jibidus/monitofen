...
class CsvMetricMapper
  ...
  def find(csv_column)
    @caching[csv_column] ||= find_metric!(csv_column)
  end

  private

  def find_metric!(csv_column)
    return nil unless MAPPING.include?(csv_column)

    Metric.find_by!(label: MAPPING[csv_column])
  end
end
